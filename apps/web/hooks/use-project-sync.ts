...
type SyncStatus = "local" | "loading" | "saved" | "pending" | "offline" | "conflict" | "deleted";
...
export function useProjectSync(projectId: string | undefined, data: AlumnoData, setData: (value: AlumnoData) => void) {
  const revision = useRef(0); const loaded = useRef(false); const pending = useRef(false); const skipNextSave = useRef(false); const deleted = useRef(false); const saveTimer = useRef<ReturnType<typeof setTimeout> | undefined>(undefined); const reconnectTimer = useRef<ReturnType<typeof setTimeout> | undefined>(undefined); const socket = useRef<WebSocket | null>(null);
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      nextSocket.onmessage = async (event) => { try { const message = JSON.parse(event.data) as { type?: string; projectId: string; revision?: number }; if (message.projectId !== projectId) return; if (message.type === "project.deleted") { stopped = true; deleted.current = true; nextSocket.close(); setStatus("deleted"); return; } if (message.revision === undefined || message.revision <= revision.current) return; if (pending.current) { setStatus("conflict"); return; } const snapshot = await getProject(projectId); if (!stopped && snapshot.revision >= revision.current) { revision.current = snapshot.revision; skipNextSave.current = true; setData(snapshot.data); setStatus("saved"); } } catch { if (!stopped) setStatus("offline"); } };
  ...
  useEffect(() => {
    if (!projectId || !loaded.current || deleted.current) return;
